...
void rrc::ue::rrc_mobility::handle_ue_meas_report(const meas_report_s& msg)
{
  ...
  for (const meas_result_eutra_s& e : eutra_report_list) {
    ...
    Info("Attempting handover????");
    Info("Target eci: %d", meas_ev.target_eci);
    Info("Cell info common not nullptr???: %d", c != nullptr);
    Info("Measurement object fields...");
    Info("Measurement id: %d", measid_it->meas_obj_id);
    //Info("Carrier freq: %d", meas_res.carrier_freq);
    //Info("Measurement Id: %d", meas_res.meas_id);
    //Info("Allowed meas: " + std::to_string(meas_ev.meas_obj.meas_obj_eutra().allowed_meas_bw.value));
    //Info("Measurement id: " + std::to_string(meas_ev.meas_obj_id));
